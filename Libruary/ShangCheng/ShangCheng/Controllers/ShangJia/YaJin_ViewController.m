...
@interface YaJin_ViewController ()<UITableViewDelegate, UITableViewDataSource>{
    NSString *tempTips;
}
...
@implementation YaJin_ViewController
...
- (void)getData {
    ...
    [[APPNetAPIManager sharedManager]request_shop_money_WithBlock:^(id data, NSError *error) {
        [self.view endLoading];
        if (data) {
            tempTips = [data stringByReplacingOccurrencesOfString:@"\"" withString:@""];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tempTips.length > 0) {
        [cell cellForYaJinWithString:tempTips];
    }
    cell.sure_pay_Block = ^(BOOL isWechat) {
        if (isWechat == YES) {
            [self.view beginLoading];
            [[APPNetAPIManager sharedManager]request_wxPay_WithTotal_price:@"" out_trade_no:@"" sj_money:tempTips andBlock:^(id data, NSError *error) {
                [self.view endLoading];
                if (data) {
            ...
            [self.view beginLoading];
            [[APPNetAPIManager sharedManager]request_alipaySign_WithTotal_price:@"" out_trade_no:@"" sj_money:tempTips andBlock:^(id data, NSError *error) {
                [self.view endLoading];
                if (data) {
