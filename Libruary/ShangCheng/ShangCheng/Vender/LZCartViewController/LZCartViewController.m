...
- (void)getShopCart{
    ...
    [[APPNetAPIManager sharedManager]request_Cart_WithMember_id:[Login_Total curLoginUser].id andBlock:^(id data, NSError *error) {
        ...
        if (data) {
            ...
            array = [ChangeNsnull changeType:array];
            for (int i = 0; i < array.count; i++) {
                NSDictionary *dic = array[i];
                LZCartModel *model = [[LZCartModel alloc]init];
                model.nameStr = dic[@"goods_name"];
                model.price = dic[@"shop_price"];
                model.number = [dic[@"goods_number"] integerValue];
                model.image = dic[@"mid_logo"];
                model.sizeStr = @"";
                model.goods_id = dic[@"goods_id"];
                model.member_id = dic[@"member_id"];
                model.shangjia_id = dic[@"shangjia_id"];
                model.goods_yfprice = dic[@"goods_yfprice"];
                model.id = dic[@"id"];
...
- (void)setupCustomBottomView {
    ...
    [backgroundView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(LZTabBarHeight);
    }];
...
- (void)setupCartView {
    ...
    [self.myTableView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(-LZTabBarHeight);
    }];
}
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            [[APPNetAPIManager sharedManager]request_delgoods_WithMember_id:[Login_Total curLoginUser].id cart_id:model.id andBlock:^(id data, NSError *error) {
                if (data) {
...
- (void)goToPayButtonClick:(UIButton*)button {
    if (self.selectedArray.count > 0) {
        ...
        for (LZCartModel *model in self.selectedArray) {
            ...
            [dic setObject:model.price forKey:@"shop_price"];
            [dic setObject:[NSString stringWithFormat:@"%zd",model.number] forKey:@"goods_number"];
            [dic setObject:model.nameStr forKey:@"goods_name"];
            [dic setObject:model.image forKey:@"mid_logo"];
            [dic setObject:model.shangjia_id forKey:@"shangjia_id"];
            [dic setObject:model.goods_yfprice forKey:@"goods_yfprice"];
            [array addObject:dic];
//            NSLog(@"选择的商品>>%@>>>%ld",model,(long)model.number);
        ...
        [[APPNetAPIManager sharedManager]request_addorder_WithGoods_id:jsonStr member_id:[Login_Total curLoginUser].id andBlock:^(id data, NSError *error) {
            if (data) {
...
//                    [self.dataArray removeAllObjects];
                ...
                dic = [ChangeNsnull changeType:dic];
                vc.contentDic = dic;
