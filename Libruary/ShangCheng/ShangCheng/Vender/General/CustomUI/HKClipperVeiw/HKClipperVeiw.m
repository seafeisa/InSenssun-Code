...
- (void)correctClipperView {
    ...
    if (width >= [UIScreen mainScreen].bounds.size.width) {
        ...
        if (scaleDistance > self.baseImgView.frame.size.height) {
            height = self.baseImgView.frame.size.height;
        }else{
            height = scaleDistance;
        }
    }else{
        height = width/self.resultImgSize.width * self.resultImgSize.height;
    }
    
    DebugLog(@"%f----%f",scaleDistance,self.baseImgView.frame.size.height);
...
- (void)setBaseImg:(UIImage *)baseImg {
    ...
    self.baseImgView.frame = CGRectMake(0, -((kScreen_Height - 64 - img.size.height) / 2), img.size.width, img.size.height);
...
- (UIImageView *)clipperView {
    if (!_clipperView) {
        ...
        CGFloat y = (self.frame.size.height - height - 64 - 32) / 2 ;
